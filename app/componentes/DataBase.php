...
class DataBase{
	...
	private function conectar(){
		...
		$this->conexao = mysqli_connect(CONFIG['host'], CONFIG['user'], CONFIG['pass'], CONFIG['database']);
		if ( !$this->conexao ) {
	...
	public function insert($table, $datas=array()){
		...
		if (mysqli_query($this->conexao, $sql)) {
			...
			$dados = [
				'msg' => "<p class=\"bg-success\"> Valores Inserido com Sucesso! </p>",
				'validator' => 1,
			]; 
		  ...
		  $dados = [
				'msg' => "<p class=\"bg-danger\"> Error: " . $sql . "<br>" . mysqli_error($this->conexao) . "</p>",
				'validator' => 0,
			];
